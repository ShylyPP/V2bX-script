...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget --no-check-certificate -N --progress=bar -O /usr/local/V2bX/V2bX-linux.zip https://github.com/ShylyPP/V2bX/releases/download/${last_version}/V2bX-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/ShylyPP/V2bX/releases/download/${last_version}/V2bX-linux-${arch}.zip"
        echo -e "开始安装 V2bX $1"
...
EOF
    ...
    curl -o /usr/bin/V2bX -Ls https://raw.githubusercontent.com/ShylyPP/V2bX-script/master/V2bX.sh
